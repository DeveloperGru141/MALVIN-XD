...
function isBotMentionedInMessage(message, botNumber) {
    ...
    if (textContent) {
        ...
        const botNames = [global.botname?.toLowerCase(), 'bot', 'ademola', 'ademola xd'];
    ...
    try {
        ...
        if (q) {
            ...
            if (action === 'on' || action === 'enable') {
                ...
                return reply("❌ Invalid option! Use: .autoread on/off", { quoted: fakevCard });
            }
        } else {
        ...
        await reply(`✅ Auto-read has been ${config.enabled ? 'enabled' : 'disabled'}!`, { quoted: fakevCard });
