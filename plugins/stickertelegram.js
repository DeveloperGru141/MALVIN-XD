...
ademola({
    ...
    try {
        ...
        if (!args[0]) {
            return await reply('⚠️ Please enter the Telegram sticker URL!\n\nExample: .tg https://t.me/addstickers/Porcientoreal');
        }

        // Validate URL format
        if (!args[0].match(/(https:\/\/t.me\/addstickers\/)/gi)) {
            return await reply('❌ Invalid URL! Make sure it\'s a Telegram sticker URL.');
        }
        ...
        try {
            ...
            for (let i = 0; i < totalStickers; i++) {
                try {
                    ...
                    const metadata = {
                        ...
                        'sticker-pack-name': settings.packname || 'Ademola XD',
                    ...
                    successCount++;
                    await delay(1000); // 1 second delay between stickers
            ...
            await reply(`✅ Successfully downloaded ${successCount}/${totalStickers} stickers from "${packName}"!`);
