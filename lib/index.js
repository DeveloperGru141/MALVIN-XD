...
async function isSudo(userId) {
    try {
        ...
        // Handle LID format - check if any sudo entry contains the phone number
        // This handles cases where the owner's LID is in the sudo list
        const settings = require('../settings');
        const ownerNumber = settings.ownerNumber;
        
        for (const sudoEntry of data.sudo) {
            if (sudoEntry && sudoEntry.includes(ownerNumber)) {
                // Check if the userId contains the same phone number (for LID format)
