...
exports.smsg = (ademolaBot, m, store) => {
    ...
    // FIX: Properly define M using the imported proto
    ...
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            // FIX: Check if M.fromObject exists before calling it
            ...
            try {
                if (M && typeof M.fromObject === 'function') {
                    ...
                    // Fallback: create a simple object if M.fromObject is not available
                    console.warn('M.fromObject is not available, using fallback object');
                    vM = m.quoted.fakeObj = {
                        key: {
                            remoteJid: m.quoted.chat,
                            fromMe: m.quoted.fromMe,
            ...
            m.quoted.copyNForward = (jid, forceForward = false, options = {}) => {
                if (vM && vM.key) {
                    ...
                    console.warn('Cannot copyNForward: vM or vM.key is undefined');
    ...
    m.copy = () => {
        try {
            if (M && typeof M.fromObject === 'function') {
                ...
                // Fallback: return the original message if M.fromObject is not available
                console.warn('M.fromObject not available for copy, returning original message');
                return exports.smsg(ademolaBot, m, store)
            }
        } catch (error) {
            console.error('Error in m.copy():', error);
            return exports.smsg(ademolaBot, m, store)
        }
    }
